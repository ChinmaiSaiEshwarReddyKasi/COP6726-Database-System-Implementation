...
void Record :: WriteHelper(FILE *fp, Schema *mySchema){
    int n = mySchema->GetNumAtts();
    Attribute *atts = mySchema->GetAtts();
...
void Record :: CreateRecord (Type type,int intSum,double doubleSum)
{

    char *recSize = new (std::nothrow) char[PAGE_SIZE];
    if (recSize == NULL)
        exit(1);
    ...
    if (bits != NULL)
        delete [] bits;
    bits = NULL;
    int recPos = sizeof (int) * (2);
    ((int *) recSize)[1] = recPos;
    if (type == Int)
    {
        *((int *) &(recSize[recPos])) = intSum;
        recPos += sizeof (int);
    }
    else
    {
        ...
        *((double *) &(recSize[recPos])) = doubleSum;
        recPos += sizeof (double);
    }
    ((int *) recSize)[0] = recPos;
    bits = new (std::nothrow) char[recPos];
    if (bits == NULL)
    {
        cout << "Out of Memory Error. Exiting...\n";
        exit(1);
    }
    memcpy (bits, recSize, recPos);
    delete [] recSize;
}
